#!/usr/bin/env python3
"""Charts for the unified SNLB Farms comprehensive feasibility study."""
...
OUT = Path(__file__).resolve().parents[1] / "charts"
...
def fig_mechanization() -> None:
    ...
    for b, v in zip(bars, vals):
        ax.text(b.get_x() + b.get_width() / 2, v + 0.6, "Baseline" if v == 0 else f"{v}%", ha="center", fontsize=9, color=G0, fontweight="bold")
    save(fig, "fig_mechanization.png")


def fig_mile12() -> None:
    # selected verified points + planning lines
    ...
    ax.axhline(22, color=G1, ls="--", lw=1.4, label="Base Regular ₦22k")
    ax.axhline(90, color=OR, ls="--", lw=1.4, label="Base Peak ₦90k")
    ax.axhline(33, color=G3, ls=":", lw=1.2, label="Upside Regular ₦33k")
    ax.axhline(145, color=RD, ls=":", lw=1.2, label="Upside Peak ₦145k")
    ax.set_xticks(range(5))
    ax.set_xticklabels(cats)
    ax.set_ylabel("₦ ’000 per 50 kg big basket")
    ax.set_title("Mile 12 Big-Basket Price Context (Selected Verified Points)")
...
def fig_cycle_pnl() -> None:
    # Base case (complete cost stack) C1–C6
    cycles = ["C1\nRegular", "C2\nPeak", "C3\nRegular", "C4\nPeak", "C5\nRegular", "C6\nPeak"]
    rev = np.array([7.9, 86.4, 118.8, 777.6, 530.3, 2678.4])
    npat = np.array([0.89, 45.7, 28.3, 421.1, 115.4, 1410.0])
    ...
    ax.set_title("Cycle Revenue & Net Profit — Base Case (₦ million)")
    ax.legend(frameon=False, fontsize=8)
    labels = [(1, 86.4, "₦86m"), (1, 45.7, "₦46m"), (3, 777.6, "₦778m"), (3, 421, "₦421m"), (5, 2678, "₦2,678m"), (5, 1410, "₦1,410m")]
    ax.set_ylim(0, 3100)
...
def fig_annual() -> None:
    ...
    rev = [94.3, 896.4, 3208.7]
    npat = [46.5, 449.4, 1525.3]
    buf = [23.3, 248.0, 1010.6]
    x = np.arange(3)
    w = 0.25
    ...
    ax.set_ylabel("₦ million")
    ax.set_title("Annual Revenue, Net Profit & Cumulative Buffer — Base Case")
    ax.legend(frameon=False, fontsize=8)
...
def fig_buffer() -> None:
    ...
    ys = [0, 23.3, 248.0, 1010.6]
    fig, ax = plt.subplots(figsize=(8.0, 3.6))
    ax.fill_between(xs, ys, color=G3, alpha=0.85)
    ax.plot(xs, ys, color=G0, lw=2.4, marker="o", ms=7)
    for x, y in zip(xs[1:], ys[1:]):
        ax.text(x, y + 40, f"₦{y:.0f}m" if y < 1000 else "₦1,011m", ha="center", fontsize=8.5, color=G0, fontweight="bold")
...
def fig_margins() -> None:
    ...
    ax.set_ylim(35, 95)
    ax.set_title("Gross & Net Margin Trend — Base Case")
...
def fig_breakeven() -> None:
    ...
    ax.set_title("Cycle 1 Break-even vs Base Regular Selling Price")
    ax.set_ylim(0, 30000)
    for b, v in zip(bars, vals):
        ax.text(b.get_x() + b.get_width() / 2, v + 600, f"₦{v:,}", ha="center", fontsize=9, fontweight="bold", color=G0)
    ax.annotate(
...
def fig_sensitivity() -> None:
    labels = [
        "Combined stress\n(yield −25%, Peak ₦55k, cost +25%)",
        "Peak price ₦90k → ₦63k (−30%)",
        "Yield −20%",
        "Regular price at ₦15,000",
        "Input cost +30%",
    ]
    # directional % vs base Y3 NPAT 1525 — approximate from model logic / downside Y3
    # Use documented study figures:
    # downside Y3 npat 387 vs 1525 = -74.6% as stacked
    vals = [-74.6, -28.0, -22.0, -8.0, -5.0]
    ...
    ax.set_xlabel("Change in Year 3 NPAT vs base case (%)")
    ax.set_title("Sensitivity — Impact on Year 3 Base-Case Net Profit")
    ax.set_xlim(-90, 10)
    save(fig, "fig_sensitivity.png")


def fig_three_case() -> None:
    labels = ["Stress case", "Base case\n(planning)", "Upside case\n(holiday + Peak prints)"]
    npv = [0.43, 2.62, 8.79]
    colors = [OR, G0, G2]
    ...
    for b, v in zip(bars, npv):
        ax.text(b.get_x() + b.get_width() / 2, v + 0.15, f"₦{v:.2f}bn", ha="center", fontsize=9, fontweight="bold", color=G0)
    ax.set_ylim(0, 10.2)
